...
| 3 | CU-80 | Configurando parámetros de detección de morosidad | Deuda | 🚧 Backend listo — `MorosidadModule` (`apps/controller/src/morosidad/`) con `GET`/`PUT /api/admin/morosidad/configuracion/:idContrato`, validación de rangos y bitácora. Los parámetros son **por contrato**: cada uno define su propio corte. **Faltan las columnas en la base** (`contrato.dias_gracia` y `contrato.umbral_suspension`, ver [`ADMIN-TABLAS-DEUDA.md`](./ADMIN-TABLAS-DEUDA.md)). El frontend es del grupo CRM |
