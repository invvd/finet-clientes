...
| 6 | CU-44 | Registrando pago confirmado con trazabilidad financiera | Núcleo de pago | ✅ Implementado — `POST /admin/pagos/confirmar`, ver [`apps/controller/docs/pagos.md`](../apps/controller/docs/pagos.md). Excepciones 2/3 trazables en `log_auditoria` (decisión: reusar la tabla existente en vez de crear `incidencia_pago`). **Fix retroactivo:** al implementar CU-46 se encontró que nunca marcaba `factura.estado` como pagada — el saldo mostrado en portal/deuda pública no reflejaba los pagos registrados. Corregido con un núcleo transaccional compartido (`aplicarPago()`) |
| 7 | CU-45 | Validando unicidad de código de transacción para evitar duplicados | Núcleo de pago | ✅ Implementado — valida antes del insert y ante condición de carrera (constraint `@unique`); rechazos y fallas de consulta quedan en `log_auditoria`, consultables vía `GET /admin/pagos/rechazados`. Aplica también a CU-46 (mismo chequeo compartido) |
| 8 | CU-46 | Incorporando abonos de recaudación externa al saldo del cliente | Núcleo de pago | ✅ Implementado — `POST /admin/pagos/abonos-externos`. Identifica el contrato solo por `codigo_abonado` (no por RUT, para evitar ambigüedad con múltiples contratos); aplica a la factura pendiente/vencida más antigua, monto debe calzar exacto (sin pagos parciales en el schema). Falta un flujo de carga masiva/batch (hoy un abono por request) |
| 9 | CU-52 | Generando comprobante de pago en formato PDF | Núcleo de pago | ✅ Implementado — se genera automáticamente dentro de `aplicarPago()` (nunca falla el pago si el comprobante falla). PDF real de referencia: ~1.5 KB, muy por debajo del límite de 500 KB. **Salvedad:** almacenamiento en disco local, no en un servicio cloud real (sin credenciales disponibles) — función `guardarComprobante()` aislada para reemplazar cuando las haya. Agregado de paso: `GET /admin/pagos` (historial de transacciones exitosas), que no existía |
| 10 | CU-53 | Enviando comprobante de pago al correo del cliente | Núcleo de pago | ✅ Implementado — se dispara automáticamente tras CU-52 (nunca falla el pago). Reintento con backoff exponencial (3 intentos, mismo patrón que `fetchWithRetry` del frontend) para la Excepción 2; Excepción 1 (sin correo) y Excepción 3 (PDF no disponible) cortan el envío sin reintentar. Nuevo método `MailService.sendComprobantePago()` — primer método de `MailService` con adjuntos |
