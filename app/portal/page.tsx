...
import { LayoutDashboard, Wifi, AlertCircle, RefreshCw, Zap } from "lucide-react";
...
export default function PortalPage() {
  ...
  const nombre = data?.cliente.nombre_completo ?? authCliente?.nombre_completo ?? "";
        ...
        <>
          ...
          {data.contratos.length === 0 ? (
            <div className="border border-[var(--color-border)] rounded-xl p-8 bg-[var(--color-background)] text-center">
              <Wifi size={32} className="mx-auto text-[var(--color-muted)]" aria-hidden />
                    ...
                    {c.plan ? (
                      <>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <Zap size={14} className="text-[var(--color-muted)]" aria-hidden />
                ...
                <ul className="space-y-2">
                  {data.tickets_recientes.map((t) => (
                    <li key={t.id_ticket} className="text-sm">
                      ...
                      <span className="text-[var(--color-muted)]"> — {t.estado}</span>
